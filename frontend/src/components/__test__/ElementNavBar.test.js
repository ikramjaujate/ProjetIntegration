...
describe('ElementNavBar', () => {
    ...
    it('Vérifier le d du svg', () => {
        ...
        const svg = wrapper.find('path');
        expect(svg.at(0).props().d).toEqual(dPath);
    })
    ...
    it("Vérifier la valeur par défaut du lien vers lequel pointe l'élément", () => {
        ...
        const reference = wrapper.find('a');
        expect(reference.at(0).props().href).toEqual('home');
    })

    it('Vérifier la valeur par défaut du texte affiché', () => {
        const wrapper = shallow(<ElementNavBar />);
        const title = wrapper.find('span');
        expect(title.text()).toEqual('Accueil');
    })

    it('Vérifier la valeur par défaut du data-icon du svg', () => {
        const wrapper = shallow(<ElementNavBar />);
        const icon = wrapper.find('svg');
        expect(icon.at(0).props()['data-icon']).toEqual('house');
    })

    it('Vérifier la valeur par défaut du classname du svg', () => {
        const wrapper = shallow(<ElementNavBar />);
        const svgClass = wrapper.find('svg');
        expect(svgClass.hasClass('fa-house')).toEqual(true) ;
    })

    it('Vérifier la valeur par défaut du viewBox du svg', () => {
        const wrapper = shallow(<ElementNavBar />);
        const svg = wrapper.find('svg');
        expect(svg.at(0).props().viewBox).toEqual('0 0 576 512');
    })
