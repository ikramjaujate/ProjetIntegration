...
module.exports = function (app, client) {
  ...
  where ME.id_member = ($1)";
    ...
    where id_member = ($1)";
        client.query(query, [idMember, name, surname], (error) => {
            if (error) {
                throw error;
            }
            response.status(200);
    ...
    where id_member = ($1) and pictures = ($2)";
        ...
        client.query(query, [idMember, photo], (error) => {
            if (error) {
                throw error;
            }
            response.status(200);
